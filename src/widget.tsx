...
class DeezerSearch extends React.Component <DeezerSearchProps, DeezerSearchState> {
  ...
  async searchDeezer(event : React.ChangeEvent<HTMLInputElement>){
    ...
    await sleep(500)
    if (current_search === this.last_search &&
      ...
      .then((response) => {
        ...
        for(const track of response.data.data){
          if (track.preview !== undefined){
            if (new_songs.find(o => o.preview === track.preview) === undefined){
              new_songs.push({title: track.title, 
                              artist: track.artist.name,
                              preview: track.preview})
            }
          }
        }
        this.setState({songs : new_songs.slice(0,Math.min(10, new_songs.length))})
      });
    }
  }
      ...
      <div>
        ...
        </form>
        {
          this.state.songs.map((song, index) => {
            return (
              <div key={`song-${index}`}>
                <button className="deezer-song-button" onClick={()=>{this.props.sendMusicURL(song.preview)}}>
...
class AudioVisualizer extends React.Component <any, AudioVisualState> {
  ...
  constructor(props : any) {
    ...
    this.updateFrequencyGraphUntilComplete = this.updateFrequencyGraphUntilComplete.bind(this)
  ...
  retrieve_audio_and_play(url : string){
    ...
    if (this.state.current_audio === null || (this.url !== '' && this.url !== url)){
      this.url = url
    ...
    if (this.state.spectrum !== null) {
      ...
      if (canvas !== null){
        ...
        if (canvas_context !== null){
          ...
          let column_width = Math.ceil(CANVAS_WIDTH/(this.state.spectrum.length * 2))
          let i = 0
          for(const freq of Array.from(this.state.spectrum)){
             canvas_context.fillStyle = "#FF0000";
             canvas_context.fillRect(column_width * i, CANVAS_HEIGHT-freq, column_width * (i+1), CANVAS_HEIGHT);
             i += 1
          }
        }
      }
